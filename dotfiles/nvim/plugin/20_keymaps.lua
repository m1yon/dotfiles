...
local explore_at_file = function()
  ...
  if path == '' or vim.fn.filereadable(path) == 0 then return MiniFiles.open() end
  MiniFiles.open(path)
end
...
local pick_files = function()
  ...
  pick.start({
    source = {
      items = function()
        return vim.fn.systemlist({ 'rg', '--files', '--color=never', '--glob', '!repos/**' })
      end,
      name = 'Files (rg)',
...
nmap_leader('ff', pick_files, 'Files')
...
nmap_leader('gr', '<Cmd>Review<CR>', 'Review')
nmap_leader('gR', '<Cmd>Review commits<CR>', 'Review commits')
...
nmap_leader('at', '<Cmd>lua require("99").tutorial()<CR>', 'Tutorial')
nmap_leader('av', '<Cmd>lua require("99").vibe()<CR>', 'Vibe')
nmap_leader('aV', '<Cmd>lua require("99").Extensions.Worker.vibe()<CR>', 'Work vibe')
