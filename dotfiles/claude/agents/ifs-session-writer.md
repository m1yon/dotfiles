...
```
{
  ...
  pending_renames: { /* SLICE 9 — local pending-state view: { <old_title>: <new_title> } for every rename_part queued. Empty {} when no rename happened (the common case). Used by part_ref resolution: a part_ref of <old> queued before the rename collapses to <new> at write time. */ },
  pending_changes: [
...
**Slice 9 — aliases + renames + backlink rewriting**:

- The skill queues `append_alias { part_ref, new_phrase }` whenever the same part surfaces with a phrasing that doesn't exactly match its current title. Apply this entry by Editing `Parts/<part_ref>.md` to append `new_phrase` to the `aliases:` frontmatter list. Append-only — even if the phrase already appears in the list, leave it (deduplicate ONLY on exact-match per case-sensitive comparison; don't reorder, don't compress, don't normalize whitespace beyond YAML's own rules). Aliases are a therapeutic artifact: the order is the chronological record of how the part has been seen.
- The skill queues `rename_part { old_title, new_title, reason? }` when the user accepts the inline rename offer at end-of-naming. The full mechanics are described under "Part-page handling — rename mechanics (slice 9)" below — read there before applying. Briefly: rename the file, append `<old_title>` to `aliases:` AFTER the new-phrase alias from the matching `append_alias`, rewrite `[[<old_title>]]` references in `Sessions/*.md` as `[[<new_title>|<old phrase as it appeared then>]]` (preserving historical phrasing), rewrite `[[<old_title>]]` references in `Parts/*.md` frontmatter as plain `[[<new_title>]]`.
- `pending_renames` (input field) is the local pending-state view — a map of `<old_title>` → `<new_title>`. Use it to resolve any `part_ref` in `pending_changes` or in input fields (`focus_part.working_title`, `re_targeted_parts[].working_title`, `re_targeted_parts[].re_targeted_from`, `record_polarization.pair[]` members, `record_protects.exile_ref` if it's a wikilink target, etc.) that still references an old title. Walk the chain: if `pending_renames["A"] = "B"` and `pending_renames["B"] = "C"`, then a `part_ref: "A"` resolves to `"C"`. The skill normally collapses these in-place when queueing, but the subagent's resolution discipline catches anything earlier-queued that still references the old title.
- The session note's frontmatter (`parts_touched`, `new_parts`, `permission_granted`, `polarization_pair`) and body (`## Parts encountered ### [[...]]` sub-sections) all use the post-rename canonical title. Resolve each part reference through `pending_renames` before rendering.
...
1. **Session note first.** `Sessions/<date> — <short-desc>.md`. Resolve every part reference in the session-note frontmatter and body through `pending_renames` before composing — `parts_touched`, `new_parts`, `permission_granted`, `polarization_pair`, `### [[<title>]]` headings, prose mentions, etc. all use the *post-rename* canonical title.
2. On success, apply remaining changes: part-page touches and `Trailheads.md` updates. **Within the part-page batch, apply `rename_part` entries FIRST** (so subsequent operations target the new file path), then `create_part` / `update_last_seen` / `append_alias` / `record_protects` / `set_left_without_resolution` / `record_polarization` / `set_status`. Then `strike_trailhead` last.
...
9. **`rename_part` (slice 9 — inline collaborative rename)**: if `pending_changes` contains a `rename_part { old_title, new_title, reason? }` entry (queued at end-of-naming when the user accepted the inline rename offer), apply it via the rename mechanics described in **"Rename mechanics (slice 9)"** below. **Apply rename_part entries FIRST within the part-page batch**, before any other entry that targets the same part (`update_last_seen`, `append_alias`, `record_protects`, `set_left_without_resolution`, `record_polarization`, `set_status`). After the rename completes, all subsequent operations target `Parts/<new_title>.md`. The skill collapses post-rename `part_ref`s to the new title via `pending_renames` resolution, so by the time you reach the Edit step on a rename-targeted page, the path resolution should be straightforward.

### Rename mechanics (slice 9)

Apply `rename_part { old_title, new_title, reason? }` entries before any other part-page operation on the same part. Order within a single rename:

**Step 1 — Rename the file.** `Parts/<old_title>.md` → `Parts/<new_title>.md`. Read the old file's contents first; on the rename, write the contents to the new path AND remove the old path (or use atomic rename if the harness supports it; otherwise Read → Write new → delete old). On collision (`Parts/<new_title>.md` already exists for some other part), abort the rename: add a `failed` entry `"rename_part: target file already exists"` and skip remaining steps. The session note is still written with the *old* title in this collision case (the `pending_renames` resolution didn't get to apply because the rename failed); add an additional `failed` informational entry for the user to reconcile.

**Step 2 — Update the renamed page's frontmatter.** Edit `Parts/<new_title>.md`:
- Append `<old_title>` to the `aliases:` list. Order: appended AFTER the new-phrase alias from any matching `append_alias { part_ref: <old_title>, new_phrase }` entry that was queued earlier in the same session (the `append_alias` is applied first per its position in `pending_changes`, then the rename step appends `<old_title>`). The `aliases:` list reads chronologically: existing aliases (oldest first) → new surfaced phrase → old canonical title. Deduplicate ONLY on exact-match; preserve order otherwise.
- Update the page's `# <title>` H1 (line 1 of body) from `<old_title>` to `<new_title>` if present.

**Step 3 — Rewrite session-note backlinks.** Glob `Sessions/*.md`. For each session note that contains `[[<old_title>]]` (in any context — frontmatter list values like `parts_touched:`, `new_parts:`, `permission_granted:`, `polarization_pair:`, `protects:`, OR body content like `### [[<old_title>]]` headings, prose mentions, etc.):
- Replace `[[<old_title>]]` with `[[<new_title>|<old phrase as it appeared then>]]`. The "old phrase as it appeared then" is `<old_title>` itself — the canonical title at the time of that session was `<old_title>`, so that's the historical phrasing that surfaces in the alias-display form. The pipe-alias syntax `[[New|Old]]` keeps the link live to the renamed page while preserving the visual phrasing of the original session.
- This applies to EVERY occurrence in EVERY session note — the canonical title is replaced everywhere it shows as `[[<old_title>]]`. Be exhaustive (use Glob + Read + Edit per file). The session note for the current session (the one the subagent just wrote) is one of those files — apply the same rewrite there if any `[[<old_title>]]` references slipped through.
- For frontmatter list-value rewrites: YAML allows `parts_touched: [[wants me to double-check everything]], [[the perfectionist]]` style — replace each occurrence in-place, preserving the YAML structure.

**Step 4 — Rewrite other-part-page backlinks.** Glob `Parts/*.md` (excluding `Parts/<new_title>.md` itself, which is the renamed page). For each part page that contains `[[<old_title>]]` (in frontmatter list values like `protects:`, `polarized_with:`, `allies:`, OR body content like backlinks):
- Replace `[[<old_title>]]` with plain `[[<new_title>]]` (NO alias form). The reasoning: these references are to the part *in the abstract* (this part is polarized with X, this part protects X), not tied to a specific moment, so the alias form would be misleading.
- Apply exhaustively — every part page that links the renamed part needs updating.

**Step 5 — Failure handling.** If any of steps 3–4 fails partway through a Glob batch (e.g. a session note's heading was hand-edited and the Edit can't find a match), add a `failed` entry for that file and continue with the remaining files. The rename is still considered successful at the file-level (step 1) and the canonical alias was preserved (step 2); leftover stale `[[<old_title>]]` links surface in Obsidian's broken-link panel and the user can fix by hand. Add a recovery-stub entry per the standard partial-failure protocol.

**Step 6 — Idempotency.** On retry: if `Parts/<new_title>.md` already exists AND `Parts/<old_title>.md` does NOT exist, treat the rename as already applied for step 1 and skip ahead to steps 2–4 (the steps that are append-only / replace-in-place are safe under exact replay). Step 2's alias append deduplicates on exact-match. Steps 3–4's link rewrites are idempotent — `[[<new_title>]]` won't get re-rewritten because no `[[<old_title>]]` remains.

**Step 7 — Multi-rename within one session (rare; `pending_renames` chain).** If `pending_renames` contains a chain `A → B → C` (i.e. two rename_part entries queued in order), apply them in queue order: A → B first (full mechanics), then B → C (full mechanics). The aliases list on the final page after both renames reads: `[<existing aliases>, <surfaced phrase>, A, B]` (A from the first rename, B from the second). Session-note backlinks land as `[[C|A]]` for original-A references (the historical phrasing was A; the alias-form syntax shows what was actually written) and `[[C|B]]` for any post-first-rename references (which would be unusual in a single session, but possible). The PRD calls this case rare; correctness matters more than economy.
...
- `rename_part { old_title, new_title, reason? }` — rename the file, append `old_title` to `aliases:`, rewrite session-note backlinks `[[old_title]]` → `[[new_title|old phrase]]` (preserve historical phrasing), rewrite other part-page backlinks `[[old_title]]` → `[[new_title]]` plain. **Full mechanics under "Rename mechanics (slice 9)" above** — apply rename_part entries FIRST within the part-page batch, before other part-page operations on the same part.
...
`part_ref` resolution: the skill collapses renames into the pending-state view (`pending_renames`) as renames are queued, so any `part_ref` queued *after* a rename already references the post-rename canonical title. However, `part_ref` queued *before* the rename still references the old title — at write time, walk `pending_renames` to resolve every `part_ref` (and every wikilink target inside `record_protects.exile_ref`, `record_polarization.pair`, etc.) to its post-rename canonical title before applying the entry. Walk the chain when transitive renames apply: `pending_renames["A"] = "B"` and `pending_renames["B"] = "C"` means a `part_ref: "A"` resolves to `"C"`. Within the part-page write batch, apply `rename_part` entries FIRST so that all subsequent operations target the new file path.
